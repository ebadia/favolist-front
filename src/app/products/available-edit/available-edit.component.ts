...
export class AvailableEditComponent implements OnInit {
  ...
  constructor(
    ...
    this._availableEditSubject.availableEditAnnounced$.subscribe(res => {
      this.editing = res
  ...
  private initForm() {
    ...
    this.form = this._fb.group({
      ...
      price: [
        data ? (data.availablePrice ? data.availablePrice : data.price) : 0
      ],
  ...
  save() {
    ...
    const obj = {
      stock: this.form.value.availableStock,
      stockOut: this.form.value.availableStockOut,
      price: this.form.value.price
    }
    this._service.update(this.product.availableId, obj).subscribe(res => {
      this._availableSubject.announceAvailable(true)
      this.cancel()
    })
  }
